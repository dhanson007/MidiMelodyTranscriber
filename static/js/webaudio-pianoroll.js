...
<div class="wac-menu">Delete</div>
<nav class="wac-menu">
    <ul class="wac-menu-items">
        <li class="wac-menu-item">
            <a href=# class="context-menu-link" name="cml" data-action="Delete">
                <i class="fa fa-eye"></i>Delete
            </a>
        </li>
        <li class="wac-menu-item">
            <a href=# class="context-menu-link" name="cml" data-action="Copy">
                <i class="fa fa-edit"></i>Copy
            </a>
        </li>
        <li class="wac-menu-item">
            <a href=# class="context-menu-link" name="cml" data-action="Paste">
                <i class="fa fa-edit"></i>Paste
            </a>
        </li>
        <li class="wac-menu-item">
            <a href=# class="context-menu-link" name="cml" data-action="Loop">
                <i class="fa fa-times"></i>Loop Selected
            </a>
        </li>
    </ul>
</nav>
    ...
    `;
            ...
            function Interval(){
                ...
                while(current+this.preload>=this.time1){
                    ...
                    if(!e || e.t>=this.markend){
                        ...
                        //const cbev={t:this.time1,g:this.time1+gmax*this.tick2time,n:e.n,st:e.t * this.tick2time};
                        const cbev={st:e.t * this.tick2time}
                        console.log("playback",this.playcallback)
                ...
                if (isFinite(this.cursor) && this.cursor > this.xoffset + this.xrange){
                    console.log("playing",this.cursor,this.xoffset,this.xrange)
                    this.xoffset = this.xoffset + this.xrange;
                    this.redraw();
                }
            }
        ...
        this.setMMLString=function(s,tempo){
            this.sequence=[];
            this.clipboard_sequence=[];
            this.tempo = tempo;
            let i,l,n,t,g,letter,octave,defo,defl,tie,evlast,measure,beat,sixteenth,accidental;
            var notes={"c":0,"d":2,"e":4,"f":5,"g":7,"a":9,"b":11};
            var max_t = 0;
            for (i = 0;i < s.length;i++) { 
                measure = parseInt(s[i][0].split(":")[0]);
                beat = parseInt(s[i][0].split(":")[1]);
                sixteenth = parseInt(s[i][0].split(":")[2]);
                t = ((measure) * 16 + (beat) * 4 + sixteenth)
                if (t > max_t){
                    max_t = t
                }
                g = parseInt(s[i][2].replace("n",""))
                letter = s[i][1][0].toLowerCase()
                n = notes[letter]
                /*notes_keys = Object.keys(notes);
                for (j = 0;j < notes_keys.length;j++){
                    if (letter == notes_keys[j]){
                        n = 
                    }
                }*/
                if (s[i][1].length > 2){
                ...
                n = (octave - this.octadj) * 12 + n
                this.sequence.push(evlast={t:t,n:n,g:g,f:0});
            }
            this.markend = max_t;    
            this.redraw();
        },
        this.get_notedata=function(){
            var bar_nbr,beat_nbr,sixteenth,octave,note_class,letter,time_string,duration,note_nbr
            var notes=["C","C#","D","Eb","E","F","F#","G","G#","A","Bb","B"];
            var all_notes = []
            console.log("tick2time",this.tick2time)
            for(let i=0;i<this.sequence.length;++i) {
                //notearr = [];
                var ev=this.sequence[i];
                bar_nbr = parseInt(ev.t / 16);
                beat_nbr = parseInt((ev.t % 16) / 4)
                sixteenth = parseInt((ev.t % 16) % 4)
                time_string = bar_nbr.toString() + ":" + beat_nbr.toString() + ":" + sixteenth.toString()
                duration = ev.g * this.tick2time
                //console.log("dur",duration,this.tick2time,ev.g)
                duration = duration.toString() 
                note_nbr = ev.n;
                octave = Math.floor(ev.n / 12) + this.octadj;
                note_class = ev.n % 12;
                letter = notes[note_class] + octave.toString()
                all_notes.push({time:time_string,note:letter,dur:duration})
            }
            return [all_notes,this.markstart,this.markend,this.cursor]
        ...
        this.getMMLString=function(){
            ...
            var octave,remainder,note_class,letter,start_time,duration,note_nbr
            ...
            for(let i=0;i<this.sequence.length;++i) {
                // notearr = [];
                var ev=this.sequence[i];
                start_time = ev.t / 16 /  60 * this.tempo; 
                duration = ev.g / 16 / 60 * this.tempo;
                note_nbr = ev.n;
                octave = Math.floor(ev.n / 12) + this.octadj;
                note_class = ev.n % 12;
                letter = notes[note_class]
                all_notes.push([start_time,duration,note_nbr,letter,octave,note_class])


                /*measure = Math.floor(ev.t / 16);
                remainder = ev.t % 16;
                beat = Math.floor(remainder / 4);
                sixteenth = remainder % 4;
                notearr = [measure.toString() + ":" + beat.toString() + ":" + sixteenth.toString() ];
                notearr.push(letter + octave.toString());
                notearr.push(ev.g.toString() + "n")
                all_notes.push(notearr)*/
        ...
        this.copySelectedNote=function(){
            const l=this.sequence.length;
            this.clipboard_sequence = []
            for(let i=l-1;i>=0;--i){
                const ev=this.sequence[i];
                if(ev.f)
                    this.clipboard_sequence.push(ev);
            }
        };
        this.pasteSelectedNote=function(){
            const l=this.clipboard_sequence.length;
            this.clipboard_sequence.sort((x,y)=>{return x.t-y.t;});
            var max_t = 0;
            var min_t = this.clipboard_sequence[0].t
            ...
            var diff = max_t - min_t;
            for(let i=l-1;i>=0;--i){
                var ev = this.clipboard_sequence[i]
                this.addNote(ev.t + ev.g,ev.n,ev.g,this.defvelo)
        ...
        this.loopSelection=function(){
            const l=this.sequence.length;
            var max_t = 0;
            var last_ev = this.sequence[this.sequence.length - 1]
            var min_t = last_ev.t;
            for(let i=l-1;i>=0;--i){
                const ev=this.sequence[i];
                if(ev.f){
...
//            el.addEventListener("contextmenu",this.bindcontextmenu,false);
        ...
        this.cancel= function(e) {
            ...
            if(this.dragging.o=="m"){
                ...
                /*if(pos.t==this.menu){
                    this.delSelectedNote();*/
                if (pos.t.getAttribute("data-action") == "Delete"){
                    this.delSelectedNote()
                } else if (pos.t.getAttribute("data-action") == "Copy"){
                    this.copySelectedNote()
                } else if (pos.t.getAttribute("data-action") == "Paste"){
                    this.pasteSelectedNote()
                }else if (pos.t.getAttribute("data-action") == "Loop"){
                    this.loopSelection()
                }
                this.redraw();
